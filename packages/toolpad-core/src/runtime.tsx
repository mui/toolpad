...
export function Placeholder({ children }: PlaceholderProps) {
  ...
  return count > 0 ? <React.Fragment>{children}</React.Fragment> : <PlaceholderWrapper />;
}
...
export function Slots({ children }: SlotsProps) {
  const nodeId = React.useContext(NodeRuntimeContext);
  if (!nodeId) {
    return <React.Fragment>{children}</React.Fragment>;
  }
  const count = React.Children.count(children);
  return count > 0 ? <SlotsWrapper>{children}</SlotsWrapper> : <Placeholder />;
}
