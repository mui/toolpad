...
const AppSandboxRoot = styled('iframe')({
  border: 'none',
  width: '100%',
});

interface AppSandboxFile {
  code: string;
  type?: 'application/javascript' | 'text/html';
}

type AppSandboxFiles = {
  [path in string]?: AppSandboxFile;
};

function isSameFile(file1: AppSandboxFile, file2: AppSandboxFile): boolean {
...
export default function AppSandbox({
  ...
  const addFiles = React.useCallback(
    async (newFiles: AppSandboxFiles, hotUpdate?: boolean) => {
  ...
  const prevFiles = React.useRef<AppSandboxFiles>(files);
  React.useEffect(() => {
    const init = async () => {
