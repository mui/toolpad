...
function getConnectionStatusIcon(status: MySQLConnectionStatus | null): React.ReactNode {
  if (!status) {
    return <SyncIcon />;
  }
...
function getConnectionStatusColor(
  status: MySQLConnectionStatus | null,
...
function withDefaults(value: Maybe<MySQLConnectionParams>): MySQLConnectionParams {
  return {
    host: '',
    port: 3306,
    user: '',
...
function ConnectionParamsInput({ value, onChange }: ConnectionEditorProps<MySQLConnectionParams>) {
  ...
  const fetchPrivate = useFetchPrivate<MySQLPrivateQuery, any>();

  const [connectionStatus, setConnectionStatus] = React.useState<MySQLConnectionStatus | null>(
    null,
  );
...
function QueryEditor({
  ...
  value: input,
  onChange: setInput,
}: QueryEditorProps<MySQLConnectionParams, MySQLQuery>) {
  ...
  const fetchPrivate = useFetchPrivate<MySQLPrivateQuery, MySQLResult>();
  const fetchServerPreview = React.useCallback(
    (query: MySQLQuery, params: Record<string, string>) =>
...
function getInitialQueryValue(): MySQLQuery {
  return {
    sql: 'SELECT NOW()',
  };
}

const dataSource: ClientDataSource<MySQLConnectionParams, MySQLQuery> = {
  displayName: 'MySQL',
