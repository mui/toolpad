...
function ComponentPropEditor<P, K extends keyof P & string>({
  ...
  const propTypeDef =
    ...
    propTypes[prop.type] as PropTypeDefinition<typeof prop.type>;

  const observableState = usePageStateObservable();
  const propValue = node.props[name];

  const [value, setValue] = React.useState(
    propValue?.type === 'binding'
      ? observableState.getValue(propValue.state)
  ...
  }, [observableState, propValue, prop.defaultValue]);
