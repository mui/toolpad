  ...
  await Promise.all(
    findPagesMarkdown(path.join(REPO_ROOT, 'docs/data')).map(async (markdown) => {
      ...
      if (
        ...
        const importStatement = `docs-toolpad/data${markdown.pathname}/core/${name}.md`;
        const demosSource = `
      ...
      `;
        ...
        if (components && components.length > 0) {
          ...
          const ${component}ApiReq = require.context(
            'docs-toolpad/translations/api-docs/${componentNameKebabCase}',
            false,
        ...
        if (hooks && hooks.length > 0) {
          ...
          const ${hook}ApiReq = require.context(
            'docs-toolpad/translations/api-docs/${hookNameKebabCase}',
            false,
